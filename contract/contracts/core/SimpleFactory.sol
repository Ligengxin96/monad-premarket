...
import {SimpleMarketCore} from "./SimpleMarketCore.sol";
...
contract SimpleFactory is ISimpleFactory {
    ...
    function createMarket() external {
        ...
        address newSimpleMarket = address(
            new SimpleMarketCore{
                salt: keccak256(abi.encodePacked(marketId, block.timestamp, block.chainid))
            }(governance, marketId)
        );
        marketInfo[marketId] = MarketInfo({
            market: newSimpleMarket,
            createTime: uint64(block.timestamp)
        });
        emit CreateMarket(marketId, newSimpleMarket);
        marketId++;
        require(newSimpleMarket != address(0));
    }
